...
from parrotspeech.model import CFM
from parrotspeech.model.dataset import DynamicBatchSampler, collate_fn
from parrotspeech.model.utils import default, exists
...
class Trainer:
    def __init__(
        ...
        if self.logger == "wandb":
            if exists(wandb_resume_id):
                init_kwargs = {"wandb": {"resume": "allow",
                                         "name": wandb_run_name, "id": wandb_resume_id}}
            else:
                init_kwargs = {
                    "wandb": {"resume": "allow", "name": wandb_run_name}}
        ...
        if self.is_main:
            self.ema_model = EMA(
                model, include_online_model=False, **ema_kwargs)
                ...
                if self.keep_last_n_checkpoints > 0:
                    ...
                    while len(checkpoints) > self.keep_last_n_checkpoints:
                        ...
                        os.remove(os.path.join(
                            self.checkpoint_path, oldest_checkpoint))
    ...
    def load_checkpoint(self):
        ...
        if "model_last.pt" in os.listdir(self.checkpoint_path):
            ...
            training_checkpoints = [f for f in all_checkpoints if f.startswith(
                "model_") and f != "model_last.pt"]
        ...
        # always a pretrained checkpoint
    ...
    def train(self, train_dataset: Dataset, num_workers=16, resumable_with_seed: int = None):
        if self.log_samples:
            from parrotspeech.inference.utils_infer import cfg_strength, load_vocoder, nfe_step, sway_sampling_coef
        ...
        total_updates = math.ceil(
            len(train_dataloader) / self.grad_accumulation_steps) * self.epochs
        ...
        self.scheduler = SequentialLR(
            self.optimizer, schedulers=[
                warmup_scheduler, decay_scheduler], milestones=[warmup_updates]
        )
        ...
        for epoch in range(skipped_epoch, self.epochs):
            ...
            progress_bar = tqdm(
                range(math.ceil(len(train_dataloader) /
            ...
            for batch in current_dataloader:
                ...
                if self.accelerator.sync_gradients:
                    ...
                    progress_bar.set_postfix(update=str(
                        global_update), loss=loss.item())
                ...
                if global_update % self.save_per_updates == 0 and self.accelerator.sync_gradients:
                    ...
                    if self.log_samples and self.accelerator.is_local_main_process:
                        ...
                        infer_text = [
                            text_inputs[0] + ([" "] if isinstance(text_inputs[0],
                                              list) else " ") + text_inputs[0]
                        ]
                        with torch.inference_mode():
                            ...
                            gen_mel_spec = generated[:, ref_audio_len:, :].permute(
                                0, 2, 1).to(self.accelerator.device)
                            ...
                            if self.vocoder_name == "vocos":
                                ...
                                gen_audio = vocoder(
                                    gen_mel_spec).squeeze(0).cpu()
                                ref_audio = vocoder(
                                    ref_mel_spec).squeeze(0).cpu()
