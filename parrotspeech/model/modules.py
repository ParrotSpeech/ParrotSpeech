...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        if self.native_rms_norm:
            ...
            x = F.rms_norm(x, normalized_shape=(
                x.shape[-1],), weight=self.weight, eps=self.eps)
        else:
...
class AttnProcessor:
    def __init__(
        self,
        # number of attention head to apply rope, None for all
...
class MMDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_norm_c = AdaLayerNorm_Final(
            context_dim) if context_pre_only else AdaLayerNorm(context_dim)
